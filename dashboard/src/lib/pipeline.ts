...
export function derivePipelineStages(normalized: RecoveryExecutionState): PipelineStageView[] {
  ...
  return [
    ...
    providerAccepted || persistedProviderAcceptance ? fromAudit("Accept", providerAccepted, "COMPLETE", truth?.basis ?? "Provider resource and execution timestamp persisted")
      : view("Accept", truth?.stage === "EXECUTION_REQUESTED" ? "ACTIVE" : executionFailed || stopped || Boolean(policyBlock) || Boolean(governorBlock) || normalized.humanReview.resolution === "REJECTED" ? "SKIPPED" : "QUEUED", truth?.stage === "EXECUTION_REQUESTED" ? truth.basis : "No provider acceptance persisted"),
