...
export function pipelineStates(detail: IncidentDetail, audit: AuditEntry[]): PipelineStageView[] {
  return derivePipelineStages(normalizeRecoveryExecution(detail, audit));
}

export function derivePipelineStages(normalized: RecoveryExecutionState): PipelineStageView[] {
  const { detail, audit } = normalized;
  ...
  const policyDecision = normalized.policy.event;
  const policyBlock = normalized.policy.resolution === "DENIED" ? policyDecision : undefined;
  const governorDecision = normalized.governor.event;
  const governorBlock = normalized.governor.resolution === "DENIED" ? governorDecision : undefined;
  ...
  const persistedProviderAcceptance = normalized.provider.accepted;
  ...
  return [
    ...
    executionFailed ? fromAudit("Execute", executionFailed, "FAILED", "Provider execution ended safely")
      : executionClaimed || action?.executedAt ? fromAudit("Execute", executionClaimed, ["EXECUTING", "RETRY_PENDING", "EXECUTION_UNCERTAIN"].includes(action?.status ?? "") ? "ACTIVE" : "COMPLETE", action?.status === "RETRY_PENDING" ? "Provider request is retry-pending; no provider acceptance is persisted" : action?.status === "EXECUTION_UNCERTAIN" ? "Provider outcome is uncertain; reconciliation is required" : action?.executedAt ? `Execution persisted ${action.executedAt}` : "Execution claimed")
    ...
    providerAccepted || persistedProviderAcceptance ? fromAudit("Accept", providerAccepted, "COMPLETE", truth?.basis ?? "Provider resource and execution timestamp persisted")
      : view("Accept", normalized.provider.awaitingReconciliation ? "ACTIVE" : executionFailed || stopped || Boolean(policyBlock) || Boolean(governorBlock) || normalized.humanReview.resolution === "REJECTED" ? "SKIPPED" : "QUEUED", normalized.provider.awaitingReconciliation ? truth?.basis ?? "Provider acceptance persisted; reconciliation is pending" : "No provider acceptance persisted"),
    normalized.reconciliation.confirmed || fixtureReconciliation ? fromAudit("Reconcile", reconciliation ?? fixtureReconciliation, "COMPLETE", truth?.basis ?? "Signed provider outcome persisted")
      : normalized.provider.awaitingReconciliation ? fromAudit("Reconcile", reconciliation, "ACTIVE", truth?.basis ?? "Provider accepted; awaiting signed reconciliation")
