...
function HistoricalWorkbench({ item }: { item: HistoricalValidationCaseResult }) {
  return <section className="mt-5 grid gap-4 xl:grid-cols-[1.1fr_.9fr]"><ConsolePanel eyebrow="Historical public-source replay" title={item.normalizedFailureClass.replaceAll("_", " ")}><div className="flex flex-wrap gap-2"><TruthBadge label={item.evidenceLabel || "HISTORICAL PUBLIC SOURCE"} /><TruthBadge label="REPLAY / NO EXECUTION" /><StateBadge value={item.result} /></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><EvidenceFact label="Case" value={item.caseId} /><EvidenceFact label="Product surface" value={item.productSurface.replaceAll("_", " ")} /><EvidenceFact label="Policy disposition" value={item.policyDisposition} /><EvidenceFact label="Safe refusal" value={item.safeRefusal ? "YES" : "NO"} /></div><Button nativeButton={false} className="mt-5" variant="outline" render={<Link href="/evaluation/historical" />}>Open replay evidence <BookOpen /></Button></ConsolePanel><ConsolePanel eyebrow="Expected invariant" title="Zero provider and customer tool path"><ul className="space-y-3">{item.expectedInvariants.map((line) => <li key={line} className="flex gap-2 text-xs leading-5 text-muted-foreground"><ShieldCheck className="mt-0.5 size-3.5 shrink-0 text-primary" />{line.replaceAll("_", " ")}</li>)}</ul><p className="mt-5 border-t border-slate-200 pt-4 text-xs text-muted-foreground">This is a provenance-linked public report transformed into a deterministic replay case. It is not a merchant transaction and cannot invoke Razorpay or customer communication.</p></ConsolePanel></section>;
}
