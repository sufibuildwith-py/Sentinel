...
type Dataset = "OPERATIONAL" | "HISTORICAL" | "SYNTHETIC" | "REPLAY";
const datasets: Array<{ id: Dataset; label: string }> = [{ id: "OPERATIONAL", label: "Operational" }, { id: "HISTORICAL", label: "Historical Razorpay" }, { id: "SYNTHETIC", label: "Synthetic evaluation" }, { id: "REPLAY", label: "Replay / shadow" }];
...
export function CaseBrowser() {
  ...
  const count = dataset === "OPERATIONAL" ? operationalRows.length : dataset === "HISTORICAL" ? historicalRows.length : dataset === "SYNTHETIC" ? syntheticRows.length : replayRows.length;
  const pageSize = 25;
  ...
  return <div><PageHeader eyebrow="Cases" title="Recovery case portfolio" description="Operational incidents, provenance-linked historical reports, synthetic safety scenarios, and shadow comparisons remain separate evidence universes." onRefresh={refresh} refreshing={incidents.isFetching || historical.isFetching || failureLab.isFetching || tower.isFetching} updated={incidents.dataUpdatedAt ? new Date(incidents.dataUpdatedAt) : undefined} />
    ...
    <div className="mt-4 flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><span className="sr-only">Search cases</span><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Search case, failure, policy, or source" className="pl-9" /></label>{dataset !== "SYNTHETIC" && dataset !== "REPLAY" && <select value={status} onChange={(event) => { setStatus(event.target.value); setPage(1); }} aria-label="Filter case status" className="h-10 rounded-lg border border-input bg-white px-3 text-sm"><option value="ALL">All states</option>{(dataset === "HISTORICAL" ? ["PASS", "PARTIAL", "FAIL"] : ["DETECTED", "INVESTIGATING", "DIAGNOSED", "PLANNING", "POLICY_REVIEW", "APPROVED", "HUMAN_REVIEW", "EXECUTING", "MONITORING", "RECOVERED", "FAILED", "STOPPED"]).map((value) => <option key={value}>{value}</option>)}</select>}</div>
    <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-muted-foreground">{count} matching cases</p><TruthBadge label={dataset === "OPERATIONAL" ? "RAZORPAY TEST MODE" : dataset === "HISTORICAL" ? "HISTORICAL PUBLIC SOURCE" : dataset === "SYNTHETIC" ? "SYNTHETIC BENCHMARK" : "SHADOW ONLY"} /></div>
...
function HistoricalTable({ rows }: { rows: Awaited<ReturnType<typeof api.historicalValidation>>["cases"] }) {
  ...
  return <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white/75"><table className="w-full min-w-[980px] text-left"><caption className="sr-only">Provenance-linked historical Razorpay failure and integration cases derived from public sources.</caption><thead><tr className="border-b border-slate-200 font-mono text-[9px] tracking-[.12em] text-slate-400 uppercase">{["Case", "Source class", "Date", "Surface", "Failure", "Policy", "Result", "Provenance"].map((label) => <th key={label} className="px-4 py-3 font-medium">{label}</th>)}</tr></thead><tbody>{rows.map((item) => { const url = safeExternalUrl(item.sourceUrl); return <tr key={item.caseId} className="border-b border-slate-100 last:border-0"><td className="px-4 py-4 font-mono text-[10px] text-primary">{item.caseId}</td><td className="px-4 py-4"><TruthBadge label={item.evidenceLabel || "HISTORICAL PUBLIC SOURCE"} /></td><td className="px-4 py-4 text-xs">{item.sourceDate}</td><td className="px-4 py-4 text-xs">{item.productSurface.replaceAll("_", " ")}</td><td className="px-4 py-4 text-xs font-medium">{item.normalizedFailureClass.replaceAll("_", " ")}</td><td className="px-4 py-4"><StateBadge value={item.policyDisposition} /></td><td className="px-4 py-4"><StateBadge value={item.result} /></td><td className="px-4 py-4">{url ? <a href={url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-xs text-primary hover:underline">Public source <ExternalLink className="size-3" /></a> : <span className="text-xs text-slate-400">Unavailable</span>}</td></tr>; })}</tbody></table></div>;
}
