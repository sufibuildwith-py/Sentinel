...
function StatusIsland() {
  ...
  return <motion.div layout className="pointer-events-none absolute left-1/2 top-2 z-20 -translate-x-1/2" aria-live="polite">
    <motion.div layout className="flex min-h-9 items-center gap-2 rounded-full border border-white/10 bg-[#11151be8] px-3 shadow-2xl backdrop-blur-xl">
...
function CommandPalette() {
  ...
  return <><Button variant="ghost" size="sm" onClick={() => setOpen(true)} aria-label="Open command palette"><Search /> <span className="hidden md:inline">Search</span><kbd className="ml-2 hidden rounded border border-white/10 px-1.5 py-0.5 text-[10px] text-muted-foreground lg:inline">⌘K</kbd></Button>
