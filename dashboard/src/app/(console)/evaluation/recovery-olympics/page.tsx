...
export default function RecoveryOlympicsPage() {
  ...
  return <div><Link href="/evaluation" className="mb-4 inline-flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground"><ArrowLeft className="size-3" />Back to Evaluation Lab</Link>
    ...
    <div className="mb-5 flex flex-wrap gap-2"><Truth label={report.truthLabel} /><Truth label={`SEED ${report.seed}`} /><Truth label={report.datasetVersion} /></div>
    {report.datasetSize === 0 ? <Empty text={report.limitations[0]} /> : <>
      ...
      <section className="glass-panel mt-4 overflow-hidden rounded-2xl"><div className="border-b border-white/[0.06] p-5"><p className="eyebrow">Same cases, visible trade-offs</p><h2 className="mt-2 text-lg font-semibold">Arm scorecard</h2></div><div className="overflow-x-auto"><table className="min-w-[1180px] w-full text-left text-xs"><thead><tr className="border-b border-white/[0.06] font-mono text-[9px] uppercase tracking-widest text-[#444444]"><th className="p-4">Arm</th><th>Net increment</th><th>Gross / natural</th><th>Cost</th><th>Increment rate (95% CI)</th><th>Refusals</th><th>Unsafe</th><th>Audit</th><th>p95</th></tr></thead><tbody>{report.arms.map((arm) => <tr key={arm.arm} className="border-b border-white/[0.04] last:border-0"><td className="p-4"><div className="flex items-center gap-2"><span className="font-mono text-primary">{arm.arm}</span><span className="font-medium">{arm.label}</span></div><span className="mt-2 inline-flex border border-white/10 px-2 py-1 font-mono text-[8px] uppercase tracking-wider text-[#888888]">{arm.methodologyLabel.replaceAll("_", " ")}</span></td><td className={arm.netIncrementalValueMinor >= 0 ? "text-[#22c55e]" : "text-[#ef4444]"}>{money(arm.netIncrementalValueMinor)}</td><td>{money(arm.grossRecoveryMinor)} / {money(arm.naturalRecoveryMinor)}</td><td>{money(arm.recoveryCostMinor)}</td><td>{percent(arm.incrementalRecoveryRate.value)} <span className="text-muted-foreground">({percent(arm.incrementalRecoveryRate.lower95)}–{percent(arm.incrementalRecoveryRate.upper95)})</span></td><td>{arm.refusals}</td><td><StateBadge value={arm.unsafeExecutions === 0 ? "PASS" : "FAILED"} /></td><td>{percent(arm.auditCompleteness)}</td><td>{arm.decisionLatencyMillis.p95}ms</td></tr>)}</tbody></table></div></section>
