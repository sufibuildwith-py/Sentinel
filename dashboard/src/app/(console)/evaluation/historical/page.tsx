...
import { ArrowLeft, ChevronLeft, ChevronRight, ExternalLink, Search, ShieldCheck } from "lucide-react";
...
export default function HistoricalValidationPage() {
  ...
  const filtered = useMemo(() => (query.data?.cases ?? []).filter((item) => `${item.caseId} ${item.sourceClass} ${item.productSurface} ${item.normalizedFailureClass} ${item.policyDisposition}`.replaceAll("_", " ").toLowerCase().includes(filter.toLowerCase())), [filter, query.data]);
  const pageSize = 25;
  const pageCount = Math.max(1, Math.ceil(filtered.length / pageSize));
  const visible = paginate(filtered, Math.min(page, pageCount), pageSize);
  ...
  return <div><Link href="/evaluation" className="mb-4 inline-flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground"><ArrowLeft className="size-3" />Back to Evaluation Lab</Link>
    ...
    <div className="mb-5 flex flex-wrap gap-2"><TruthBadge label={report.truthLabel} /><TruthBadge label="HISTORICAL DERIVED REPLAY" /><TruthBadge label={`CORPUS ${report.corpusVersion}`} /><TruthBadge label={`SHA-256 ${report.manifestSha256.slice(0, 12)}`} /></div>
    {report.acceptedPublicSourceCases === 0 ? <Empty text={report.limitations[0]} /> : <>
      ...
      <section className="glass-panel mt-4 rounded-2xl p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><p className="eyebrow">Case explorer</p><p className="mt-1 text-xs text-muted-foreground">Showing {visible.length} of {filtered.length} matching cases; all {report.cases.length} are browseable.</p></div><label className="relative"><span className="sr-only">Filter historical cases</span><Search className="absolute left-3 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" placeholder="Filter case, surface, failure" value={filter} onChange={(event) => { setFilter(event.target.value); setPage(1); }} /></label></div><div className="mt-5 grid gap-3 md:grid-cols-2">{visible.map((item) => { const sourceUrl = safeExternalUrl(item.sourceUrl); return <article key={item.caseId} className="rounded-xl border border-slate-200 bg-white/70 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[10px] text-primary">{item.caseId}</p><h3 className="mt-2 text-sm font-semibold">{item.normalizedFailureClass.replaceAll("_", " ")}</h3></div><StateBadge value={item.result} /></div><dl className="mt-4 grid grid-cols-2 gap-3 text-[10px]"><Fact label="Source" value={item.sourceClass.replaceAll("_", " ")} /><Fact label="Date" value={item.sourceDate} /><Fact label="Surface" value={item.productSurface.replaceAll("_", " ")} /><Fact label="Policy" value={item.policyDisposition} /></dl><div className="mt-4 flex items-center justify-between gap-3"><TruthBadge label={item.evidenceLabel || "HISTORICAL PUBLIC SOURCE"} />{sourceUrl ? <a href={sourceUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 font-mono text-[9px] uppercase tracking-wider text-primary hover:underline">Public source <ExternalLink className="size-3" /></a> : <span className="font-mono text-[9px] text-slate-400">SOURCE URL UNAVAILABLE</span>}</div></article>; })}</div><div className="mt-5 flex items-center justify-between border-t border-slate-200 pt-4"><p className="font-mono text-[9px] text-slate-400">PAGE {Math.min(page, pageCount)} / {pageCount}</p><div className="flex gap-2"><Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}><ChevronLeft />Previous</Button><Button variant="outline" size="sm" disabled={page >= pageCount} onClick={() => setPage((value) => Math.min(pageCount, value + 1))}>Next<ChevronRight /></Button></div></div></section>
