...
export default function IncidentsPage() {
  ...
  return <div><PageHeader eyebrow="Incident operations" title="Revenue incidents" description="Search every detected anomaly and follow its evidence-backed recovery state." onRefresh={() => void query.refetch()} refreshing={query.isFetching} updated={query.dataUpdatedAt ? new Date(query.dataUpdatedAt) : undefined} />
    ...
    {query.isLoading ? <div className="space-y-2">{Array.from({ length: 5 }, (_, i) => <Skeleton key={i} className="h-20 rounded-xl" />)}</div> : query.error ? <ErrorState error={query.error} retry={() => void query.refetch()} /> : rows.length === 0 ? <div className="py-16 text-center font-mono text-xs text-[#444444]"><p className="tracking-[0.2em] uppercase">No incidents detected</p><p className="mt-2">System is monitoring. Revenue is protected.</p></div> : <div className="overflow-hidden rounded-xl border border-white/8 bg-card/65">
      <div className="hidden grid-cols-[1.35fr_.75fr_.8fr_.75fr_1fr_.75fr_32px] gap-4 border-b border-white/8 px-4 py-3 text-[10px] font-semibold tracking-[.12em] text-muted-foreground uppercase lg:grid"><span>Failure type</span><span>Status</span><span>Amount at risk</span><span>Priority</span><span>Strategy</span><span>Policy</span><span /></div>
      {rows.map((incident) => {
        const recovered = incident.status === "RECOVERED" || incident.latestOutcome === "RECOVERED";
        return <motion.div layoutId={`incident-${incident.incidentId}`} key={incident.incidentId}><Link href={`/incidents/${incident.incidentId}`} className="grid gap-3 border-b border-white/6 px-4 py-4 transition-colors last:border-0 hover:bg-white/[.025] lg:grid-cols-[1.35fr_.75fr_.8fr_.75fr_1fr_.75fr_32px] lg:items-center lg:gap-4"><div><p className="text-sm font-semibold text-[#f5f5f5]">{incident.type.replaceAll("_", " ")}</p><p className="mt-1 font-mono text-[10px] text-[#444444]">{shortId(incident.incidentId)}</p></div><StateBadge value={incident.status} /><div><span className="lg:hidden eyebrow">Amount at risk · </span><span className={`font-mono text-sm font-semibold ${recovered ? "text-[#22c55e]" : "text-[#ef4444]"}`}>{money(incident.amountAtRiskMinor)}</span></div><p className="text-xs text-muted-foreground"><span className="lg:hidden eyebrow">Priority · </span>{incident.severity}</p><p className="text-xs text-[#444444]">{incident.strategy?.replaceAll("_", " ") ?? "Not planned"}</p><StateBadge value={incident.policyDecision} /><ArrowRight className="hidden size-4 text-muted-foreground lg:block" /></Link></motion.div>;
      })}
    </div>}
  </div>;
}
