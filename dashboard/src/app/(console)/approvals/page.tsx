...
function ApprovalCard({ item }: { item: Approval }) {
  ...
  return <div className="glass-panel rounded-2xl p-5 sm:p-6"><div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[10px] tracking-[0.16em] text-[#888888] uppercase">Incident {shortId(item.incidentId)}</p><h2 className="mt-2 text-lg font-semibold text-[#f5f5f5]">{item.incidentType.replaceAll("_", " ")}</h2></div><span className="rounded-full border border-[#f59e0b]/40 px-2.5 py-1 text-[11px] font-semibold text-[#f59e0b]">Human required</span></div>
